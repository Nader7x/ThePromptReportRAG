...
TEXT_BASED_TECHNIQUES = [
    ...
    PromptingTechnique(
        ...
        related_techniques=["Chain-of-Thought (CoT)", "Analogical Prompting", "Step-Back Prompting", "Thread-of-Thought (ThoT)", "Tabular Chain-of-Thought (Tab-CoT)"]
    ),
...
AGENT_TYPES = [
    ...
    AgentType(
        ...
        example_techniques=["Verify-and-Edit", "Demonstrate-Search-Predict (DSP)", "Interleaved Retrieval guided by Chain-of-Thought (IRCoT)", "Iterative Retrieval Augmentation (FLARE, IRP)"],
...
EVALUATION_FORMATS = [
    EvaluationFormat(
        format_type="Styling (XML, JSON)",
        description="Structured output formats for evaluation results."
    ),
    EvaluationFormat(
    ...
    EvaluationFormat(
        format_type="Binary Score (Yes/No, True/False)",
        description="Simple binary judgments for evaluation."
    ),
    EvaluationFormat(
...
PROMPT_ENGINEERING_TECHNIQUES = [
    ...
    PromptEngineeringTechnique(
        ...
        process_steps=["Generate initial prompts from exemplars", "Score generated prompts", "Create variations (e.g., via prompt paraphrasing)", "Iterate until desiderata reached"]
    ),
    ...
    PromptEngineeringTechnique(
        ...
        process_steps=["Pass batch of inputs through template", "Pass output, ground truth, and prompt into criticism prompt", "Generate new prompts from criticisms", "Use bandit algorithm to select one"]
    ),
...
def search_techniques_by_keyword(keyword: str) -> List[PromptingTechnique]:
    ...
    for tech in TEXT_BASED_TECHNIQUES:
        if (keyword in tech.technique_name.lower() or 
            keyword in tech.description.lower() or 
            keyword in tech.benefits.lower() or
            keyword in tech.how_to_apply.lower()):
            results.append(tech)
    return results
