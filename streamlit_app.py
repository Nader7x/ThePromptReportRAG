...
def call_api(endpoint: str, method: str = "GET", data: Dict = None) -> Dict:
    ...
    try:
        ...
        return {"error": error_msg, "original_exception": str(e)}
...
page = st.sidebar.selectbox(
    "Choose a page", ["🏠 Home", "✨ Prompt Enhancement", "� Analytics"]  # Simplified for working features
)
...
if page == "🏠 Home":
    ...
    with st.form("prompt_form"):
        user_prompt = st.text_area(
            "Enter your prompt:", height=100, placeholder="E.g., Help me write a professional email to my boss..."
        )
        ...
        try:
            ...
            available_techniques = ["Auto-detect"] + [t.technique_name for t in TEXT_BASED_TECHNIQUES[:10]]
        except ImportError:
        ...
        technique_hint = st.selectbox("Technique hint (optional):", available_techniques)
    ...
    if submitted and user_prompt:
        with st.spinner("Processing your prompt..."):
            ...
            is_success = (
                result
                and result.get("success") is True  # Must be explicitly True
                and result.get("enhanced_prompt")  # Must have enhanced prompt
                and result.get("enhanced_prompt").strip()
            )  # Must not be empty
    ...
    df_usage = pd.DataFrame(
        {"Date": dates, "Prompts Enhanced": prompts_enhanced, "Searches Performed": searches_performed}
    )
    fig = px.line(df_usage, x="Date", y=["Prompts Enhanced", "Searches Performed"], title="Daily Usage Metrics")
    ...
    fig_pie = px.pie(
        values=list(technique_usage.values()), names=list(technique_usage.keys()), title="Technique Usage Distribution"
    )
